export class TabMask {
	constructor(option) {
		...
		let navHeight = !option.navHeight && option.navHeight != 0 ? (uni.getSystemInfoSync().statusBarHeight + 44) : option
			.navHeight
	...
	creatPlusMask(navHeight, tabbarHeight, opacity) {
		...
		for (let i = 1; i <= 10; i++) {
			...
			let navView = this.creatView({
				height: `${navHeight}px`,
				top: 0
			}, opa)
			let tabbarView = this.creatView({
